...
const DICT = {
  ...
  en: {
    ...
    outerwear: 'Outerwear', bottoms: 'Bottoms', accessories: 'Accessories', other: 'Other',
    catalog: 'Catalog', view_all: 'View All', new_arrivals: 'New Arrivals',
    sold_out: 'Sold Out', in_stock: 'In Stock', add_to_cart: 'Add to Cart',
    no_size: 'Size Unavailable', color: 'Color', size: 'Size', size_guide: 'Size Guide',
    wishlist: 'Wishlist', in_wishlist: 'In Wishlist', add_to_wishlist: 'Add to Wishlist',
    cart: 'Cart', empty_cart: 'Cart is empty', checkout: 'Checkout',
    total: 'Total', delivery_details: 'Proceed to Payment', tracking: 'My Orders',
...
function MainApp() {
  ...
  const [adminTab, setAdminTab] = useState('analytics'); // Default to analytics to show changes
  ...
  useEffect(() => {
    ...
    // Load analytics visits only for admin to save reads
    let unsubVisits = () => {};
  ...
  useEffect(() => {
    if (isReady && showLoader) {
      setFadeLoader(true); // Start fade out
      const timer = setTimeout(() => setShowLoader(false), 1200); // Wait for transition to complete
      return () => clearTimeout(timer);
    }
  }, [isReady, showLoader]);
