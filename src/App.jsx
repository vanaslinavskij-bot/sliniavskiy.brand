                    ...
                    {adminTab === 'inventory' && (() => {
                       ...
                       return (
                          <section className="space-y-6 animate-in fade-in">
                             {/* HEADER З КНОПКОЮ УВІМКНЕННЯ/ВИМКНЕННЯ */}
                             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-black border border-white/10 p-6 shadow-xl mb-2">
                                <div>
                                   <h2 className="text-xl font-black uppercase tracking-widest text-white mb-1">Керування складом</h2>
                                   <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-black">
                                      {isInvEnabled ? 'Автоматичний облік активовано' : 'Система складу деактивована (Ручний режим)'}
                                   </p>
                                </div>
                                <button 
                                   onClick={async () => {
                                      try {
                                        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'general'), 
                                          { isInventoryEnabled: !isInvEnabled }, { merge: true });
                                        showToast(!isInvEnabled ? '✅ Склад увімкнено!' : '⚠️ Склад вимкнено (Ручне керування)');
                                      } catch (e) { showToast('❌ Помилка налаштувань'); }
                                   }}
                                   className={`flex items-center gap-2 px-6 py-3 font-black uppercase tracking-widest text-[10px] transition-all border shadow-lg ${isInvEnabled ? 'bg-red-500/10 text-red-500 border-red-500/30 hover:bg-red-500 hover:text-white' : 'bg-green-500/10 text-green-500 border-green-500/30 hover:bg-green-500 hover:text-white'}`}
                                >
                                   <Power size={14} />
                                   {isInvEnabled ? 'Вимкнути склад' : 'Увімкнути склад'}
                                </button>
                             </div>

                             {!isInvEnabled ? (
                                <div className="p-16 text-center border border-white/5 bg-zinc-900/20 backdrop-blur-sm">
                                   <div className="w-16 h-16 mx-auto mb-6 border border-white/10 bg-black rounded-full flex items-center justify-center text-zinc-600 shadow-inner">
                                      <Power size={28} className="opacity-50" />
                                   </div>
                                   <h3 className="text-lg font-black uppercase tracking-widest text-white mb-3">Склад призупинено</h3>
                                   <p className="text-[10px] text-zinc-500 uppercase font-black tracking-[0.2em] max-w-md mx-auto leading-relaxed">
                                      Автоматичне списання та перевірка залишків вимкнені. <br/>
                                      Ви можете вручну змінювати статус "В наявності" в редакторі товарів.
                                   </p>
                                </div>
                             ) : (
                                <div className="space-y-8">
                                   ...
                                   <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                      <div className="bg-white/5 border border-white/5 p-5">
                                         <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest mb-1">Товарів на складі</p>
                                         <p className="text-xl font-black text-white">{totalStockQty} шт.</p>
                                      </div>
                                      <div className="bg-white/5 border border-white/5 p-5">
                                         <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest mb-1">Загальна вартість</p>
                                         <p className="text-xl font-black text-[#d4af37]">{totalStockValue.toLocaleString()} ₴</p>
                                      </div>
                                      <div className="bg-white/5 border border-white/5 p-5">
                                         <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest mb-1">Критичний залишок</p>
                                         <p className="text-xl font-black text-red-500">{lowStockCount}</p>
                                      </div>
                                   </div>
                                   ...
                                   <div className="bg-black/40 border border-white/10 p-4 flex flex-col md:flex-row gap-4">
                                      <div className="relative flex-1">
                                         ...
                                         <input 
                                           ...
                                           className="w-full bg-black border border-white/10 pl-11 pr-4 py-3 text-xs focus:border-white outline-none text-white"
                                         />
                                      </div>
                                      <div className="flex gap-2">
                                         <select 
                                           ...
                                           className="bg-black border border-white/10 px-4 py-3 text-[10px] uppercase font-black tracking-widest outline-none focus:border-white text-white appearance-none min-w-[150px]"
                                         >
                                         ...
                                         <div className="flex bg-black border border-white/10 p-1">
                                            {['all', 'low', 'out'].map(f => (
                                               <button 
                                                 ...
                                                 className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest transition-all ${inventoryFilter === f ? 'bg-white text-black' : 'text-zinc-500 hover:text-white'}`}
                                               >
                                                  {f === 'all' ? 'Всі' : f === 'low' ? 'Дефіцит' : 'Немає'}
                                               </button>
                                            ))}
                                         </div>
                                      </div>
